...
if selection in games:
    ...
    print(f"win64 folder: {win64_folder}")
...
"""
    ...
    with open(timer_script_path, "w", encoding="utf-8") as f:
        f.write(timer_code)
    ...
    subprocess.run([
        ...
        "--noconsole",
        "--name", exe_name,
        "--distpath", win64_folder,
        "--workpath", os.path.join(win64_folder, "build"),
        timer_script_path
    ], cwd=win64_folder)

    exe_path = os.path.join(win64_folder, f"{exe_name}.exe")
    ...
    if os.path.exists(timer_script_path):
